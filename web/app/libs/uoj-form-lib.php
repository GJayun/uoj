...
EOD;
		...
		public function addInput($name, $type, $label_text, $default_value, $validator_php, $validator_js) {
			$default_value = htmlspecialchars($default_value);
			$html = <<<EOD
<div id="div-$name" class="field">
	<label for="input-$name">$label_text</label>
	<input type="$type" name="$name" id="input-$name" value="$default_value" />
...
<div id="div-$name" class="field">
	...
	<div class="col-sm-3">
		<select class="ui fluid dropdown" id="input-content" name="$name">
...
<div id="div-$name">
	...
	<select class="ui fluid dropdown" id="input-{$name}" name="$name">
...
EOD
		...
		public function printHTML() {
			...
			echo '<form action="', $_SERVER['REQUEST_URI'], '" method="post" class="ui form" id="form-', $this->form_name, '"', $form_entype_str, '>';
			echo HTML::hiddenToken();
			echo $this->main_html;
			
			if (!$this->no_submit) {
				if (!isset($this->submit_button_config['align'])) {
					$this->submit_button_config['align'] = 'left';
				}
				...
				if (!isset($this->submit_button_config['class_str'])) {
					$this->submit_button_config['class_str'] = 'ui submit button grey';
				}
				echo '<button type="submit" id="button-submit-', $this->form_name, '" name="submit-', $this->form_name, '" value="', $this->form_name, '" class="', $this->submit_button_config['class_str'], ' floated ', $this->submit_button_config['align'], '">', $this->submit_button_config['text'], '</button>';
			}
