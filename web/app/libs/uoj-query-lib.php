...
function queryTags($search) {
    $search = DB::escape($search);
    return DB::selectAll('select * from tags where en like "%' . $search . '%" or zh_cn like "%' . $search . '%"');
}
function queryTag($id) {
    return DB::selectFirst("select * from tags where id = $id", MYSQLI_ASSOC);
}
