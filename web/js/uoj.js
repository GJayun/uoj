...
function showCommentReplies(id, replies) {
	...
	replies.forEach(function (reply) {
		$('<div id="' + 'comment-' + reply.id + '" class="_item_1dxav_49"></div>').append(
			'<div class="_avatar_1dxav_61">' +
			'<a href="' + uojHome + '/user/profile/' + reply.poster + '">' +
			'<img class="ui image" src="' + reply.avatar + '" alt="avatar" />' +
			'</a>' +
			'</div>'
		).append(
			$('<div class="_bubble_1dxav_70"></div>').append(
				$('<div class="ui block top attached header _header_1dxav_21"></div>').append(
					$('<div class="_headerContents_1dxav_95"></div>').append(
						'<div class="_left_1dxav_100">' +
						'<div class="_triangle_1dxav_170"></div>\n' +
						'<div class="_username_1dxav_111"><span>' + getUserLink(reply.poster, reply.poster_rating) + '</span></div>' +
						'<span class="_commentedOn_1dxav_115">@' + reply.post_time + '</span>' +
						'</div>'
					).append(
						$('<div class="_right_1dxav_142"></div>').append(
		...
		).uoj_highlight().appendTo("#replies-" + id);
	})
}
